...
public class Rate {
    ...
    public BigDecimal calculate(Period periodStay) {
        ...
        CarParkKind type = this.kind;
        BigDecimal amount =  this.hourlyNormalRate.multiply(BigDecimal.valueOf(normalRateHours)).add(
                this.hourlyReducedRate.multiply(BigDecimal.valueOf(reducedRateHours)));
        if (type == CarParkKind.VISITOR)
        {
        	amount = amount.subtract(BigDecimal.valueOf(8.00)).divide(BigDecimal.valueOf(2.00));
        	return amount;
        }
        else if (type == CarParkKind.MANAGEMENT)
        {
        	if(amount.compareTo(BigDecimal.valueOf(3.00)) == -1)
        	{
        		amount = BigDecimal.valueOf(3.00);
        		return amount;
        	}
        }
        else if (type == CarParkKind.STUDENT)
        {
        	if(amount.compareTo(BigDecimal.valueOf(5.50)) == 1)
        	{
        		amount = amount.subtract(amount.divide(BigDecimal.valueOf(4.00)));
        		return amount;
        	}
        }
        else if (type == CarParkKind.STAFF)
        {
        	if(amount.compareTo(BigDecimal.valueOf(16.00)) == 1)
        	{
        		amount = BigDecimal.valueOf(16.00);
        		return amount;
        	}
        }
        return amount;
